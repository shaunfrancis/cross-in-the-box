...
import styles from './SenateSearchSection.module.css';
import { stateToSlug, partyIdToDisplayId } from 'src/lib/USA';

export default function SenateSearchSection( {searchInputRef} : {searchInputRef? : RefObject<HTMLInputElement>} ){
    const [results, setResults] = useState<SearchResults | null>();
    const [displayCount, setDisplayCount] = useState<number>(15);
    const [currentQuery, setCurrentQuery] = useState<string>("");
    const [status, setStatus] = useState<string>("");

    const handler = useRef(new RegionSearchHandler(Endpoint + "/search/usa/", "/senate"));

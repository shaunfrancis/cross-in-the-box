...
import styles from './SenateStatePage.module.css';
import { constituencyToSlug, partyIdToDisplayId, slugToLookupSlug } from "src/lib/UK";
import SenateSidebar from "./SenateSidebar/SenateSidebar";
import { electionType, subidLabels } from "src/constants/USA";

export default function SenateStatePage( { slug } : { slug : string } ){
            ...
            if(data.tree.find(t => t.region_id == event.region.id && !t.direct_successor)){
                data.tree.filter(t => t.region_id == event.region.id && !t.direct_successor).forEach( treeBranch => {
                    succeededByNodes.push(
                        <Link href={'/usa/senate-elections/state/' + constituencyToSlug(treeBranch.title)} className={styles["abolished-link"] + " unstyled"}>
    ...
    return ( <>
        ...
        <section id="hero">
            <a href="/usa/senate-elections/" className="breadcrumb">
                ...
                <span>US Senate Elections</span>
            </a>

            <h1>{region.title}</h1>
            <HeroNav items={heroNavItems} />
        </section>
        <RegionPage sidebar={<SenateSidebar region={region} />}>

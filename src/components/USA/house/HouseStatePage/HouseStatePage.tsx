...
export default function HouseStatePage( { slug } : { slug : string } ){
            ...
            if(data.tree.find(t => t.region_id == event.region.id && !t.direct_successor)){
                data.tree.filter(t => t.region_id == event.region.id && !t.direct_successor).forEach( treeBranch => {
                    succeededByNodes.push(
                        <Link href={'/usa/house-elections/district/' + constituencyToSlug(treeBranch.title)} className={styles["abolished-link"] + " unstyled"}>
    ...
    return ( <>
        ...
        <section id="hero">
            <a href="/usa/house-elections/" className="breadcrumb">
                ...
                <span>US House Elections</span>
            </a>

            <h1>{region.title}</h1>
            <HeroNav items={heroNavItems} />
        </section>
        <RegionPage sidebar={<HouseSidebar region={region} />}>

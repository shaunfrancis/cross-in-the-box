...
$region = API\SlugLookupService::call(["uk", $slug, "scotland"]);
...
if(!empty($data['events'][0]['region']['title']) && $data['events'][0]['region']['title'] != $region['title']){
    ...
    $_headInjections[] = sprintf('<link rel="canonical" href="https://crossinthebox.com/uk/scottish-parliament/constituency/%s" />', $newSlug);
    $_headInjections[] = sprintf('<script>history.replaceState(null,"","%s");</script>', $newSlug);
}

$_title[] = $region['title'];
$_description[] = "Browse Scottish Parliament election results and data for the {$region['title']}" . (str_ends_with($region['title'], "region") ? "." : " constituency.");
$_breadcrumb = ['title' => $region['title'], 'path' => 'uk/scottish-parliament/constituency/' . UK\regionToSlug($region['title'])];
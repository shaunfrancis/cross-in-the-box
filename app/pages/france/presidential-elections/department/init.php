...
$live_events = API\LiveEventsService::call(["france"]);
if(empty($live_events['error']) && count($live_events) > 0) $regionParams = ['live' => TRUE];
$data = API\RegionService::call(["france", $region['id']], $regionParams ?? []);
...
$_breadcrumb = ['title' => $region['title'], 'path' => 'france/presidential-elections/department/' . France\regionToSlug($region['title'])];
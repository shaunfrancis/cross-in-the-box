...
window.addEventListener('DOMContentLoaded', () => {
    [...document.querySelectorAll('.ConstituencyMap')].forEach( async constituencyMap => {
        const id = constituencyMap.getAttribute('data-region-id');
        if(!id) return constituencyMap.remove();
        ...
        try{
            ...
            const geoSource = await fetch('/public/geojson/uk/' + id + '.json').then(response => response.json());
            if(!geoSource) return;
            ...
            const map = new OLMap({
                target: constituencyMap.querySelector('.ConstituencyMap__map'),
                ...
                controls: [
                    ...
                    new ZoomToExtent({ 
                        ...
                        label: "",
                        className: "ol-zoom-extent zoom-to-extent"
                    }),
                    new Attribution({
                        label: "©"
                    })
